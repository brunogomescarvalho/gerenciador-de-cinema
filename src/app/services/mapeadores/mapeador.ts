...
export class Mapeador {
  ...
  public filmeDetalhes(obj: any) {
    return new Filme(
      ...
      obj.vote_average,
      obj.credits.cast
        .filter((x: any) => x.profile_path != null)
        .map(
          (elenco: any) =>
            new Pessoa(
              elenco.id,
              elenco.name,
              this.imgUrl + elenco.profile_path
            )
        )
        .slice(0, 10),
      obj.credits.crew
        .filter((x: any) => x.job == 'Producer')
        .map(
          (produtor: any) =>
            new Pessoa(
              produtor.id,
              produtor.name,
              this.imgUrl + produtor.profile_path
            )
        )
        .slice(0, 10),
      obj.credits.crew
        ...
        .map(
          (diretor: any) =>
            new Pessoa(
              diretor.id,
              diretor.name,
              this.imgUrl + diretor.profile_path
            )
        )
        .slice(0, 10),
      obj.vote_count,
  ...
  public mapearPessoa(obj: any) {
    return new Pessoa(
      ...
      obj.credits?.cast
        ...
        .map((filme: any) => this.filmeResumido(filme))
        .concat(
          obj.credits.crew
            .filter((poster: any) => poster.poster_path != null)
            .map((filme: any) => this.filmeResumido(filme))
      ...
      obj.birthday,
      obj.deathday,
      obj.images?.profiles.map(
        (img: any) => (img.file_path = this.imgUrl + img.file_path)
      )
    );
  }
  ...
  public mapearListaFilmes(lista: any[]): Filme[] {
    return lista
      ...
      .map((filme: any) => {
        return this.filmeResumido(filme);
      });
  }
  ...
  public mapearPesquisa(obj: any[]) {
    ...
    const pessoas = obj.filter(
      (item) => item.media_type === 'person' && item.profile_path != null
    );
